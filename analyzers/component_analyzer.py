...
class ComponentAnalyzer:
    ...
    async def analyze_all(
        ...
        for sym in indexer.symbol_table.get_by_kind(SymbolKind.COMPONENT):
            comp = self._analyze_component(
                sym,
                api_by_file.get(sym.file, []),
                form_by_file.get(sym.file, [])
            )
    ...
    def _analyze_component(self, symbol, api_ids: list, form_ids: list) -> ComponentInfo:
        ...
        for p in symbol.metadata.get('parameters', []):
            if p.name not in ('props', 'children', '_'):
                props.append(FieldInfo(
                    name=p.name,
                    field_type=p.type or 'any',
        ...
        if props_match:
            for m in re.finditer(r'(\w+)(?:\s*=\s*[^,}]+)?', props_match.group(1)):
                ...
                if name not in [p.name for p in props] and name not in ('children',):
                    props.append(FieldInfo(name=name, field_type='any', required='=' not in m.group(0)))
    ...
    def _extract_state(self, source: str) -> list[str]:
        ...
        # React useState
        for m in re.finditer(r'const\s+\[\s*(\w+)\s*,\s*\w+\s*\]\s*=\s*useState', source):
            state.append(m.group(1))

        # React useReducer
        for m in re.finditer(r'const\s+\[\s*(\w+)\s*,\s*\w+\s*\]\s*=\s*useReducer', source):
            state.append(m.group(1))

        # Zustand/Recoil selectors
        for m in re.finditer(r'const\s+(\w+)\s*=\s*use\w*(?:Store|State|Selector)', source):
            state.append(m.group(1))
