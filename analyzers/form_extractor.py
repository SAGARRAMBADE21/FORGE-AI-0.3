...
from core.types import FormInfo, FormField
...
class FormExtractor:
        ...
        if defaults:
            for m in re.finditer(r'(\w+)\s*:', defaults.group(1)):
                fields.append(FormField(
                    name=m.group(1),
                    field_type=self._infer_type(m.group(1))
                ))

        # Register calls
        for m in re.finditer(r'register\s*\(\s*[\'"](\w+)[\'"](?:\s*,\s*(\{[^}]*\}))?', content):
            name = m.group(1)
            opts = m.group(2) or ""
    ...
    def _extract_html_form(self, form_content: str, file: str, full_content: str) -> FormInfo | None:
        """Extract HTML form."""
        fields = []

        for m in re.finditer(r'<input[^>]*name\s*=\s*[\'"](\w+)[\'"][^>]*>', form_content, re.I):
            name = m.group(1)
            tag = m.group(0)
        ...
        for m in re.finditer(r'<textarea[^>]*name\s*=\s*[\'"](\w+)[\'"]', form_content, re.I):
            fields.append(FormField(name=m.group(1), field_type='textarea'))

        for m in re.finditer(r'<select[^>]*name\s*=\s*[\'"](\w+)[\'"]', form_content, re.I):
    ...
    def _infer_type(self, name: str) -> str:
        ...
        hints = {
            ...
            'tel': 'tel', 'date': 'date', 'age': 'number', 'price': 'number',
            'amount': 'number', 'quantity': 'number', 'url': 'url'
        }
