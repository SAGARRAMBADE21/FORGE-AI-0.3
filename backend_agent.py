...
# Integration module for external services (Stripe, S3, AI/ML)
from integration import IntegrationManager, AIMLGenerator
...
class BackendAgent:
    ...
    def __init__(self, project_root: Path, config: TemplateConfig | None = None):
        ...
        # External Integrations (Stripe, S3, AI/ML)
        self._integration_manager = IntegrationManager()
        self._aiml_generator = AIMLGenerator()
    ...
    def _register_actions(self):
        ...
        self._runtime.register_action(
            "extract_api_contracts", self._action_extract_api_contracts
        )
        ...
        self._runtime.register_action(
            "validate_endpoint", self._action_validate_endpoint
        )
        self._runtime.register_action(
            "generate_controller_method", self._action_generate_controller_method
        )
        self._runtime.register_action(
            "generate_service_method", self._action_generate_service_method
        )
        self._runtime.register_action("update_routes", self._action_update_routes)
        self._runtime.register_action(
            "generate_validation", self._action_generate_validation
        )
        # Add model actions
        self._runtime.register_action("validate_model", self._action_validate_model)
        self._runtime.register_action(
            "update_prisma_schema", self._action_update_prisma_schema
        )
        self._runtime.register_action(
            "generate_repository", self._action_generate_repository
        )
        self._runtime.register_action("generate_service", self._action_generate_service)
        self._runtime.register_action(
            "generate_controller", self._action_generate_controller
        )
        self._runtime.register_action(
            "generate_migration", self._action_generate_migration
        )
    ...
    def _assess_project_complexity(self) -> str:
        ...
        if self.config:
            ...
            if "payment" in str(features).lower() or "billing" in str(features).lower():
                complexity_score += 2
            if (
                "real-time" in str(features).lower()
                or "websocket" in str(features).lower()
            ):
                complexity_score += 2
    ...
    async def generate_backend(
        ...
        if self._generation_mode == GenerationMode.MULTI_AGENT:
            ...
            if progress:
                ...
                progress(
                    "mode_selection",
                    f"Auto-selected {mode_name} mode (complexity: {complexity})",
                )

            logger.info(
                f"Hybrid mode: Project complexity={complexity}, using {'multi-agent' if use_multi_agent else 'single-agent'}"
            )
        ...
        plan = await self._planner.create_plan(
            TaskType.GENERATE_BACKEND, {"project_root": str(self.root)}
        )
        ...
        if not success:
            return GenerationResult(
                success=False, errors=["Backend generation failed"], files=[]
            )
        ...
        return GenerationResult(
            success=True, files=[], stats={"message": "Generation completed"}
        )
    ...
    async def add_model(
        ...
        plan = await self._planner.create_plan(
            TaskType.ADD_MODEL, {"model": {"name": name, "fields": fields}}
        )
        ...
        return GenerationResult(
            success=success, files=[], errors=[] if success else ["Failed to add model"]
        )
    ...
    async def sync_with_frontend(
        ...
        plan = await self._planner.create_plan(TaskType.SYNC_FRONTEND, {})
        ...
        return GenerationResult(
            success=success, files=[], errors=[] if success else ["Failed to sync"]
        )
    ...
    async def _action_infer_models(self, params: dict) -> list[InferredModel]:
        ...
        evidence_map = await self._evidence_collector.collect_all(
            self._frontend_analysis
        )
    ...
    async def _action_infer_relations(self, params: dict) -> list[InferredRelation]:
        ...
        relations = await self._relationship_inferrer.infer_relationships(
            self._architecture.models, self._architecture.api_resources
        )

        self._architecture.relations = relations
        return relations

    async def _action_extract_api_contracts(
        self, params: dict
        ...
        resources = await self._api_extractor.extract_contracts(
            self._frontend_analysis.api_calls, self._architecture.models
        )
    ...
    async def _action_analyze_auth(self, params: dict) -> AuthRequirements:
        ...
        if (
            not self._frontend_analysis
            or not self._auth_analyzer
            or not self._architecture
        ):
            raise RuntimeError("Frontend not analyzed")

        auth = await self._auth_analyzer.analyze(
            self._frontend_analysis.auth, self._architecture.api_resources
        )
    ...
    async def _action_design_schema(self, params: dict):
        ...
        schema = await self._schema_designer.design_schema(
            self._architecture.models, self._architecture.relations
        )
    ...
    async def _action_generate_controller_method(self, params: dict) -> bool:
        ...
        endpoint = params.get("endpoint", {})
        logger.info(
            f"Generated controller method for {endpoint.get('method')} {endpoint.get('path')}"
        )
    ...
    async def _action_generate_service_method(self, params: dict) -> bool:
        ...
        endpoint = params.get("endpoint", {})
        logger.info(
            f"Generated service method for {endpoint.get('method')} {endpoint.get('path')}"
        )
    ...
    async def _action_update_routes(self, params: dict) -> bool:
        ...
        logger.info(
            f"Updated routes for {endpoint.get('method', '')} {endpoint.get('path', model.get('name', ''))}"
        )
    ...
    async def _action_generate_validation(self, params: dict) -> bool:
        ...
        endpoint = params.get("endpoint", {})
        logger.info(
            f"Generated validation schema for {endpoint.get('method')} {endpoint.get('path')}"
        )
    ...
    async def _generate_backend_multi_agent(
        ...
        context = GenerationContext(
            ...
            api_resources=(
                self._architecture.api_resources if self._architecture else []
            ),
    ...
    def set_generation_mode(self, mode: GenerationMode):
        ...
        self._use_multi_agent = (
            mode == GenerationMode.MULTI_AGENT or mode == GenerationMode.HYBRID
        )
    ...
    def enable_multi_agent(self, enabled: bool = True):
        ...
        self._generation_mode = (
            GenerationMode.MULTI_AGENT if enabled else GenerationMode.SINGLE_AGENT
        )
