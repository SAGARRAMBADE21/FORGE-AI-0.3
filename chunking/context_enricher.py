...
from core.utils import count_tokens, extract_keywords
...
class ContextEnricher:
    ...
    def _enrich_chunk(
        self, chunk: Chunk, symbol_lookup: Dict[str, Symbol], contents: Dict[str, str]
    ) -> bool:
        """
    ...
    def _add_parent_context(
        self, chunk: Chunk, symbol_lookup: Dict[str, Symbol]
    ) -> bool:
    ...
    def _add_related_symbols(
        ...
        function_calls = set(re.findall(r"\b([a-zA-Z_][a-zA-Z0-9_]*)\s*\(", content))

        # Find type references (capitalized identifiers)
        type_refs = set(re.findall(r"\b([A-Z][a-zA-Z0-9_]*)\b", content))
        ...
        for name in function_calls:
            if name in symbol_lookup:
                related.append({"name": name, "type": "function_call"})

        for name in type_refs:
            if name in symbol_lookup:
                related.append({"name": name, "type": "type_reference"})
    ...
    def _add_semantic_tags(self, chunk: Chunk) -> bool:
        ...
        # I/O operations
        if any(
            kw in content for kw in ["read", "write", "open", "close", "fetch", "axios"]
        ):
    ...
    def _enhance_keywords(self, chunk: Chunk) -> bool:
        ...
        for keyword in keywords:
            ...
            weighted_keywords.append(
                {"keyword": keyword, "weight": round(weight, 2), "count": count}
            )
    ...
    def _extract_docstring(self, content: str) -> str:
        ...
        python_match = re.search(
            r'^\s*["\']{{3}}(.*?)["\']{{3}}', content, re.MULTILINE | re.DOTALL
        )
        if python_match:
    ...
    def enrich_chunk_with_context_window(
        self, chunk: Chunk, file_content: str, window_lines: int = 5
    ) -> Chunk:
        """
