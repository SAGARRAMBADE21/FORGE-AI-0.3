...
from core.types import Symbol, Chunk, CrossFileContext, SymbolKind
...
class CrossFileContextBuilder:
    ...
    def get_context_for_chunk(self, chunk: Chunk, file: str) -> CrossFileContext:
        ...
        ctx.imported_by = list(self._imported_by.get(file, set()))[:self.max_cross_refs]
        ...
        if symbol_id:
            ...
            ctx.dependencies = list(
                self._call_graph.get(symbol_id, set())
            )[:self.max_cross_refs]
        ...
        if file in self._type_dependencies:
            ...
            if not hasattr(chunk, 'metadata'):
                chunk.metadata = {}
            chunk.metadata['type_dependencies'] = list(
                self._type_dependencies[file]
            )[:self.max_cross_refs]
    ...
    def _build_call_graphs(
        ...
        for file, symbols in symbols_by_file.items():
            for sym in symbols:
                sym_id = sym.metadata.get('id') if hasattr(sym, 'metadata') else sym.name
    ...
    def _extract_type_dependencies(self, content: str, lang: Language) -> List[str]:
        ...
        if lang in (Language.TYPESCRIPT, Language.TSX):
            ...
            types.extend(
                m.group(1) for m in re.finditer(
                    r'(?:interface|type)\s+([A-Z][a-zA-Z0-9_]*)', 
                    content
                )
            )
    ...
    def _extract_function_calls(self, source: str) -> List[str]:
        ...
        keywords = {
            'if', 'while', 'for', 'switch', 'catch', 'print', 
            'len', 'range', 'typeof', 'new', 'delete'
        }
    ...
    def _get_file_exports(self, file: str, chunk: Chunk) -> List[str]:
        ...
        if all_match:
            exports.extend(
                name.strip().strip('\'"') 
                for name in all_match.group(1).split(',')
            )
