...
from core.types import Chunk, ChunkType, CrossFileContext, Symbol, SymbolKind
from core.utils import count_tokens, extract_keywords, generate_id
...
class DynamicChunker:
    ...
    def build_graphs(
        ...
        for file, symbols in symbols_by_file.items():
            ...
            lang = (
                symbols[0].metadata.get("language", Language.UNKNOWN)
                if symbols
    ...
    def chunk_file(
        self, content: str, file: str, symbols: list[Symbol], language: Language
    ) -> list[Chunk]:
        lines = content.split("\n")
        if not symbols:
        ...
        if chunks and chunks[0].start_line > 0:
            header = self._create_header(
                content, lines, file, language, chunks[0].start_line
            )
            if header:
    ...
    def _create_symbol_chunk(
        self, sym: Symbol, content: str, lines: list[str], file: str, lang: Language
    ) -> Chunk | None:
    ...
    def _create_header(
        self, content: str, lines: list[str], file: str, lang: Language, first_line: int
    ) -> Chunk | None:
        ...
        return Chunk(
            ...
            content=chunk_content,
            chunk_type=(
                ChunkType.FILE_HEADER.value
                if hasattr(ChunkType.FILE_HEADER, "value")
                else "file_header"
            ),
            file=file,
            start_line=0,
    ...
    def _chunk_by_size(
        self, content: str, file: str, lang: Language, lines: list[str]
    ) -> list[Chunk]:
        chunks = []
        current_start = 0
        current_tokens = 0
    ...
    def _get_cross_file(self, chunk: Chunk, file: str) -> CrossFileContext:
        ...
        ctx.imports = list(self._import_graph.get(file, set()))[
            : settings.chunking.max_cross_refs
        ]
        ctx.imported_by = list(self._imported_by.get(file, set()))[
            : settings.chunking.max_cross_refs
        ]
        if chunk.symbol_id:
            ctx.callees = list(self._call_graph.get(chunk.symbol_id, set()))[
                : settings.chunking.max_cross_refs
            ]
            ctx.callers = list(self._reverse_calls.get(chunk.symbol_id, set()))[
                : settings.chunking.max_cross_refs
            ]
        return ctx
