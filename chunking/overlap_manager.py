...
class OverlapManager:
    ...
    def add_overlap_to_chunks(
        self, chunks: List[Chunk], lines: List[str]
    ) -> List[Chunk]:
        """
    ...
    def _add_overlap(self, prev: Chunk, curr: Chunk, lines: List[str]) -> None:
        ...
        overlap_start = max(prev.end_line - overlap_lines + 1, prev.start_line)
    ...
    def _calculate_overlap_size(
        self, prev: Chunk, curr: Chunk, lines: List[str]
    ) -> int:
        """
        ...
        estimated_lines = max(
            ...
            min(
                self.overlap_tokens // 15, self.max_overlap_lines
            ),  # ~15 tokens per line estimate
        )
        ...
        overlap_lines = self._adjust_to_boundary(prev, overlap_lines, lines)

        return overlap_lines

    def _adjust_to_boundary(
        self, chunk: Chunk, target_lines: int, lines: List[str]
    ) -> int:
        """
        ...
        Returns:
            Adjusted line count
        """
        start_search = max(chunk.end_line - target_lines - 3, chunk.start_line)
        ...
        for i in range(end_search, start_search - 1, -1):
            ...
            for marker in boundary_markers:
                if marker in line:
                    ...
                    if abs(lines_from_end - target_lines) < abs(
                        best_boundary - target_lines
                    ):
    ...
    def _should_merge(self, chunk1: Chunk, chunk2: Chunk) -> bool:
        ...
        if gap <= 2:  # Very close chunks
            ...
            combined_tokens = chunk1.metadata.get("tokens", 0) + chunk2.metadata.get(
                "tokens", 0
            )
    ...
    def _merge_chunks(self, chunk1: Chunk, chunk2: Chunk, lines: List[str]) -> Chunk:
        ...
        Returns:
            Merged chunk
        """
        from core.utils import extract_keywords, generate_id
