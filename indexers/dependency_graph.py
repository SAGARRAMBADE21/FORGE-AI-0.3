...
class DependencyGraph:
    ...
    def __init__(self):
        ...
        self._imports: dict[str, set[str]] = defaultdict(set)  # file -> imported modules
        self._imported_by: dict[str, set[str]] = defaultdict(set)  # module -> importing files
        
        # Symbol-level dependencies
        self._calls: dict[str, set[str]] = defaultdict(set)  # symbol_id -> called symbol names
        self._called_by: dict[str, set[str]] = defaultdict(set)  # symbol_name -> caller symbol names
        self._type_refs: dict[str, set[str]] = defaultdict(set)  # symbol_id -> referenced type names

    def build_from_files(self, contents: dict[str, str], languages: dict[str, Language]):
    ...
    def build_call_graph(self, symbols: list[Symbol]):
        ...
        for symbol in symbols:
            if symbol.kind not in (SymbolKind.FUNCTION, SymbolKind.METHOD, 
                                   SymbolKind.HOOK, SymbolKind.COMPONENT):
                continue
    ...
    def build_type_refs(self, symbols: list[Symbol]):
        ...
        type_names = {
            ...
            if s.kind in (SymbolKind.CLASS, SymbolKind.INTERFACE, 
                         SymbolKind.TYPE_ALIAS, SymbolKind.STRUCT)
        }
    ...
    def _extract_imports(self, content: str, language: Language) -> list[str]:
        ...
        if language in (Language.TYPESCRIPT, Language.TSX, Language.JAVASCRIPT, Language.JSX):
            # ES6 imports
