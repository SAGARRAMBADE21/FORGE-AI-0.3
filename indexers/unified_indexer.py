...
from config.settings import EXTENSION_MAP, Language, settings
from core.types import FileInfo, IndexStats, ParseError, Symbol, SymbolKind
...
class UnifiedIndexer:
    ...
    async def _index_file(
        self, file_path: Path
    ) -> tuple[FileInfo, list[Symbol], list[ParseError]] | None:
        """Index a single file."""
        try:
    ...
    async def update_file(self, file_path: Path) -> bool:
        ...
        self.dependency_graph.build_from_files(
            {relative_path: content}, {relative_path: file_info.language}
        )
