...
from core.types import (
    AuthPattern, AuthType, AuthRequirements, AuthStrategy,
    PermissionModel, RoleDefinition, PermissionDefinition,
    OAuthProviderConfig, ApiResourceContract
)
...
class AuthRequirementsAnalyzer:
    ...
    async def analyze(
        ...
        permission_model, roles, permissions = await self._infer_permissions(api_resources)

        # Build JWT/session config
        jwt_config = self._build_jwt_config() if strategy in (AuthStrategy.JWT, AuthStrategy.HYBRID) else {}
        session_config = self._build_session_config() if strategy in (AuthStrategy.SESSION, AuthStrategy.HYBRID) else {}
        ...
        for file_info in self._indexer.file_index.all_files():
            ...
            if 'google' in content.lower() and 'signin' in content.lower():
                if 'google' not in detected:
                    detected.add('google')
                    providers.append(OAuthProviderConfig(
                        name='google',
            ...
            # GitHub
            if 'github' in content.lower() and ('oauth' in content.lower() or 'signin' in content.lower()):
        ...
        for resource in api_resources:
            ...
            for endpoint in resource.endpoints:
                ...
                for perm in endpoint.permissions:
                    if perm.lower() in ('admin', 'user', 'moderator', 'editor', 'viewer'):
    ...
    async def _detect_roles_from_code(self) -> set[str]:
        ...
        for file_info in self._indexer.file_index.all_files():
            ...
            for match in re.finditer(r'type\s+Role\s*=\s*[\'"](\w+)[\'"](?:\s*\|\s*[\'"](\w+)[\'"])*', content):
    ...
    def _build_roles(
        ...
        for role_name in role_names:
            ...
            if role_name == 'user':
                ...
                role_perms = [p.name for p in permissions if ':read' in p.name or ':create' in p.name]
    ...
    async def _detect_mfa(self) -> bool:
        ...
        for file_info in self._indexer.file_index.all_files():
            content = self._indexer.get_file_content(file_info.path)
            if content and ('2fa' in content.lower() or 'mfa' in content.lower() or 'totp' in content.lower()):
                return True
        return False
