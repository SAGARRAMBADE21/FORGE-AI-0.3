...
from core.types import (
    Symbol, SymbolKind, NavRequest, 
    CallHierarchyResult, CallHierarchyItem, 
    IncomingCall, OutgoingCall, Range, Position
)
    ...
    def _get_incoming_calls(self, symbol: Symbol) -> list[IncomingCall]:
        ...
        for caller_name in callers:
            ...
            for caller in caller_symbols:
                if caller.kind in (SymbolKind.FUNCTION, SymbolKind.METHOD, 
                                  SymbolKind.HOOK, SymbolKind.COMPONENT):
                    sites = self._find_call_sites(caller, symbol.name)
                    
                    incoming.append(IncomingCall(
    ...
    def _get_outgoing_calls(self, symbol: Symbol) -> list[OutgoingCall]:
        ...
        for callee_name in callees:
            ...
            for callee in callee_symbols:
                if callee.kind in (SymbolKind.FUNCTION, SymbolKind.METHOD, 
                                  SymbolKind.HOOK, SymbolKind.COMPONENT):
                    sites = self._find_call_sites(symbol, callee.name)
                    
                    outgoing.append(OutgoingCall(
