...
class ReferenceFinder:
    ...
    async def find_references(
        self, request: NavRequest, include_definition: bool = True
    ) -> ReferenceResult:
        ...
        return ReferenceResult(symbol=symbol, references=references)

    def _find_all_references(
        self, symbol: Symbol, include_definition: bool
    ) -> list[Reference]:
    ...
    def _determine_reference_kind(
        self, line: str, col: int, name: str
    ) -> ReferenceKind:
        ...
        if (
            ":" in before
            or "<" in before
            or "extends" in before
            or "implements" in before
        ):
    ...
    def _get_container(self, file: str, line: int) -> str | None:
        ...
        for symbol in symbols:
            if (
                ...
                and symbol.range.start.line <= line <= symbol.range.end.line
            ):
                return symbol.name
