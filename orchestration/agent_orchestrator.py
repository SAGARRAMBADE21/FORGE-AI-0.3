...
class BackendSubAgent:
    ...
    async def send_message(
        self, to_agent: AgentRole, message_type: MessageType, content: Dict[str, Any]
    ) -> AgentMessage:
    ...
    async def request_help(
        self, from_agent: AgentRole, request: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Request help from another specialized agent"""
        message = await self.send_message(
            from_agent, MessageType.REQUEST, {"request": request, "context": context}
        )
        return message.content

    async def propose_solution(
        self, to_agent: AgentRole, solution: Dict[str, Any]
    ) -> bool:
        """Propose a solution for another agent to review"""
        response = await self.send_message(to_agent, MessageType.PROPOSAL, solution)
