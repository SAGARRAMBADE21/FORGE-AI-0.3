...
import re
from core.types import Comment, Docstring, Range, Position
...
class CommentExtractor:
    ...
    def _get_patterns(self, lang: Language) -> list[tuple[str, str]]:
        if lang == Language.PYTHON:
            return [(r'#[^\n]*', 'line'), (r'""".*?"""', 'doc'), (r"'''.*?'''", 'doc')]
        elif lang in (Language.JAVASCRIPT, Language.TYPESCRIPT, Language.TSX, Language.JSX,
    ...
    def _extract_raw(self, content: str, lang: Language) -> str | None:
        if lang == Language.PYTHON:
            ...
            if m:
                return m.group(1).strip()
        elif lang in (Language.JAVASCRIPT, Language.TYPESCRIPT, Language.TSX, Language.JSX, Language.JAVA):
            m = re.match(r'^\s*/\*\*(.*?)\*/', content, re.DOTALL)
            if m:
                doc = m.group(1)
    ...
    def _parse_docstring(self, raw: str, lang: Language) -> Docstring:
        ...
        if lang in (Language.JAVASCRIPT, Language.TYPESCRIPT, Language.TSX, Language.JSX, Language.JAVA):
            doc.params = dict(re.findall(r'@param\s+(?:\{[^}]+\}\s+)?(\w+)\s+(.+?)(?=@|\Z)', raw, re.DOTALL))
            ...
            if args_m:
                for pm in re.finditer(r'\s+(\w+)(?:\s*\([^)]+\))?\s*:\s*(.+)', args_m.group(1)):
