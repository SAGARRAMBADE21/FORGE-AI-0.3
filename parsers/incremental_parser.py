...
class IncrementalParser:
    ...
    def parse_content(
        self, content: str, file_path: str, language: Language | None = None
    ) -> ParseResult:
        if language is None:
    ...
    def parse_incremental(
        self, file_path: str, content: str, changes: list[IncrementalChange]
    ) -> ParseResult:
        language = self._registry.detect_language(file_path)
        ...
        symbols = list(
            self._extract_symbols(new_tree.root_node, content, file_path, language)
        )
    ...
    def _extract_symbols(
        ...
        if lang in (
            ...
            yield from self._extract_js_ts(
                node, content, file, lang, parent_id, parent_name
            )
        elif lang == Language.PYTHON:
            yield from self._extract_python(
                node, content, file, lang, parent_id, parent_name
            )
        elif lang == Language.GO:
            yield from self._extract_go(
                node, content, file, lang, parent_id, parent_name
            )
        elif lang == Language.RUST:
            yield from self._extract_rust(
                node, content, file, lang, parent_id, parent_name
            )
        elif lang == Language.JAVA:
            yield from self._extract_java(
                node, content, file, lang, parent_id, parent_name
            )
        elif lang in (Language.C, Language.CPP):
            yield from self._extract_c(
                node, content, file, lang, parent_id, parent_name
            )
        else:
            for child in node.children:
                yield from self._extract_symbols(
                    child, content, file, lang, parent_id, parent_name
                )
    ...
    def _extract_js_ts(
        ...
        if node.type in ("function_declaration", "generator_function_declaration"):
            ...
            if name_node:
                ...
                sym.is_async = (
                    b"async" in content[node.start_byte : node.start_byte + 20].encode()
                )
                ...
                if body:
                    yield from self._extract_js_ts(
                        body, content, file, lang, sym.id, sym.name
                    )
                return
            ...
            for child in node.children:
                if child.type == "variable_declarator":
                    ...
                    if (
                        ...
                        and value_node
                        and value_node.type
                        in ("arrow_function", "function", "function_expression")
                    ):
                        sym = self._create_symbol(
                            child,
                            name_node,
                        ...
                        sym.is_async = (
                            b"async"
                            in content[
                                value_node.start_byte : value_node.start_byte + 20
                            ].encode()
                        )
                        sym.signature = self._signature(sym)
                        name = name_node.text.decode()
                        ...
                        kind = (
                            SymbolKind.CONSTANT
                            if b"const" in node.text[:10]
                            else SymbolKind.VARIABLE
                        )
                        yield self._create_symbol(
                            child,
                            name_node,
                            content,
                            file,
                            ...
                            if h.type == "extends_clause":
                                ...
                                for t in h.children:
                                    if t.type in ("identifier", "type_identifier"):
                                        sym.metadata["interfaces"].append(
                                            t.text.decode()
                                        )
                yield sym
                body = node.child_by_field_name("body")
        ...
        if node.type in ("export_statement", "export_declaration"):
            ...
            if decl:
                for sym in self._extract_js_ts(
                    decl, content, file, lang, parent_id, parent_name
                ):
    ...
    def _extract_python(
        ...
        if node.type in ("function_definition", "async_function_definition"):
            ...
            if name_node:
                ...
                sym = self._create_symbol(
                    node, name_node, content, file, lang, kind, parent_id, parent_name
                )
                ...
                if body and body.children:
                    ...
                    if first.type == "expression_statement" and first.children:
                        ...
                        if expr.type == "string":
                            sym.docstring = self._comments.extract_docstring(
                                expr.text.decode(), lang
                            )
                yield sym
                if body:
                    yield from self._extract_python(
        ...
        if node.type == "assignment" and parent_id is None:
            ...
            if left and left.type == "identifier":
                ...
                yield self._create_symbol(
                    node, left, content, file, lang, kind, parent_id, parent_name
                )
                return
    ...
    def _extract_rust(
        ...
        if node.type == "impl_item":
            ...
            if type_node:
                ...
                if body:
                    yield from self._extract_rust(
                        body, content, file, lang, parent_id, type_node.text.decode()
                    )
            return
    ...
    def _parse_unsupported(
        self, content: str, file: str, lang: Language
    ) -> ParseResult:
        import re
                        ...
                        Symbol(
                            ...
                            metadata={
                                ...
                                "selection_range": Range(
                                    Position(i, m.start(1)), Position(i, m.end(1))
                                ),
                                "source": line,
                                "language": lang,
                            },
    ...
    def _create_symbol(
        ...
        selection = Range(
            Position(name_node.start_point[0], name_node.start_point[1]),
            Position(name_node.end_point[0], name_node.end_point[1]),
        )
        qualified = f"{file}:{parent_name}.{name}" if parent_name else f"{file}:{name}"
        source = content[node.start_byte : node.end_byte]
        is_exported = node.parent and node.parent.type in (
            "export_statement",
            "export_declaration",
        )
    ...
    def _js_params(self, node: Node) -> list[Parameter]:
        ...
        for child in pn.children:
            if child.type in ("required_parameter", "optional_parameter", "identifier"):
                ...
                params.append(
                    Parameter(name=child.children[1].text.decode(), rest=True)
                )
        return params
    ...
    def _parse_js_param(self, node: Node) -> Parameter | None:
        ...
        name_node = node.child_by_field_name("pattern") or node.child_by_field_name(
            "name"
        )
        ...
        return Parameter(
            name=name,
            param_type=type_ann or "",
            default=default,
            optional=node.type == "optional_parameter" or default is not None,
        )
    ...
    def _signature(self, sym: Symbol) -> str:
        params = ", ".join(
            f"{p.name}: {p.param_type}" if p.param_type else p.name
            ...
            for sym in sorted_syms:
                ...
                if 0 <= start - end <= 2:
                    ...
                    if comment.kind == "doc" and not sym.doc:
                        ...
                        sym.doc = self._comments.extract_docstring(
