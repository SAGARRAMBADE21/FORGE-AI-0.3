...
from core.types import Migration, DatabaseSchema
...
class MigrationSafetyChecker:
    ...
    DANGEROUS_OPERATIONS = [
        ...
        ('ALTER COLUMN.*NOT NULL', 'medium', 'Adding NOT NULL may fail on existing data'),
    ...
    def generate_rollback(self, migration: Migration) -> str | None:
        ...
        for match in re.finditer(r'ALTER TABLE\s+(\w+)\s+ADD\s+COLUMN\s+(\w+)', up_sql, re.IGNORECASE):
            table_name = match.group(1)
            column_name = match.group(2)
            rollback_parts.append(f'ALTER TABLE {table_name} DROP COLUMN {column_name};')
