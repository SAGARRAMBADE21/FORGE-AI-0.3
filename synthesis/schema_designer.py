...
class SchemaDesigner:
    ...
    def _field_to_column(
        self, field: InferredField, model_name: str
        ...
        return DatabaseTable(
            ...
            indexes=[
                DatabaseIndex(
                    ...
                    columns=[
                        f"{relation.source_model.lower()}Id",
                        f"{relation.target_model.lower()}Id",
                    ],
                    unique=True,
                )
            ],
    ...
    def _add_foreign_keys(
        self, tables: list[DatabaseTable], relations: list[InferredRelation]
    ) -> list[DatabaseTable]:
        ...
        for rel in relations:
            if rel.relation_type in (
                InferredRelationType.MANY_TO_ONE,
                InferredRelationType.ONE_TO_ONE,
            ):
                            ...
                            table.foreign_keys.append(
                                {
                                    ...
                                    "on_delete": (
                                        "CASCADE" if rel.cascade_delete else "SET NULL"
                                    ),
                                }
                            )
                            break
