...
class ServiceArchitect:
    ...
    async def design_architecture(
        ...
        logger.info(
            f"Designed {len(repositories)} repositories, {len(services)} services"
        )

        return ServiceArchitecture(
            repositories=repositories, services=services, events=events
        )

    def _design_repository(
        self, model: InferredModel, relations: list[InferredRelation]
    ) -> RepositoryDefinition:
            ...
            if field.unique and field.name != "id":
                methods.append(
                    RepositoryMethod(
                        ...
                        return_type=f"{model.name} | null",
                        parameters=[
                            {"name": field.name, "type": self._field_to_ts_type(field)}
                        ],
                        query_type="select",
                    )
                )
        ...
        return RepositoryDefinition(
            name=f"{model.name}Repository", model=model.name, methods=methods
        )
    ...
    def _endpoint_to_service_method(
        self, endpoint, model: InferredModel | None, relations: list[InferredRelation]
    ) -> ServiceMethod:
        ...
        if model:
            if endpoint.method == "GET":
                repo_calls.append(
                    "findById"
                    if "id" in [p.name for p in endpoint.path_params]
                    else "findMany"
                )
            elif endpoint.method == "POST":
    ...
    def _design_events(
        self, resource: ApiResourceContract, model: InferredModel | None
    ...
    def _get_method_name(self, endpoint) -> str:
        ...
        has_id = any(
            p.name == "id" or p.name.endswith("Id") for p in endpoint.path_params
        )
    ...
    def _find_model(
        self, name: str | None, models: list[InferredModel]
    ) -> InferredModel | None:
        """Find model by name."""
        if not name:
            return None
