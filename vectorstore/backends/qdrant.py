...
class QdrantBackend:
    ...
    async def initialize(self):
        ...
        self._client = QdrantClient(path=str(settings.vectorstore.persist_dir / "qdrant"))
    ...
    async def search(
        ...
        if filters:
            conditions = []
            if "file" in filters:
                conditions.append(FieldCondition(key="file", match=MatchValue(value=filters["file"])))
        ...
        for hit in results:
            ...
            chunk = Chunk(
                ...
                chunk_type=ChunkType(payload["chunk_type"]) if payload.get("chunk_type") else ChunkType.CODE_BLOCK,
                ...
                symbol_kind=SymbolKind(payload["symbol_kind"]) if payload.get("symbol_kind") else None,
                language=Language(payload["language"]) if payload.get("language") else Language.UNKNOWN,
